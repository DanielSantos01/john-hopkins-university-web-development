...
(function(global){
    ...
    ajaxUtils.sendGetRequest = function(requestURL, responseHandler){ //função a ser chamada quando ocorrer um evento (um request) - recebe uma url e a função a ser executada como resposta
        ...
        request.onreadystatechange = function(){ //passa o endereço da função para que ela seja executada quando o estado mudar(basicamente diz o que acontece quando receber a resposta do servidor)
            handleResponse(request, responseHandler); //função que realmente será executada/chamada
        };
    ...
    function handleResponse(request, responseHandler){ //fnção realmente executada quando o request for aceito
        if(request.readyState == 4 && request.status == 200){ //verifica o status da requisição
            responseHandler(request); //aplica a função desejada no request object
        }
    };
